...
struct NeuralNetwork{
	...
	vector<lf> predict(vector<lf> in){
		...
		for(int i=0;i<numNodes[numLayers-1];i++) rt[i] = node[numLayers-1][i].ff;
		return rt;
	}
	...
	pair<vector<vector<lf>>, vector<vector<vector<lf>>>> calc_der(vector<lf> y){
		...
		for(int j=0;j<numNodes[numLayers-1];j++){
			delta[numLayers-1][j] = activ_der(node[numLayers-1][j].ss)*cost_der(y[j], node[numLayers-1][j].ff);
		}
...
void learn_data(){
	freopen("learn_ev.txt", "r", stdin);
	freopen("nn_ev.txt", "w", stdout);
	NN.init_rand();
	int valid = 0;
	int batch_size = 1;
	for(int i=0;i<150000/batch_size;i++){
